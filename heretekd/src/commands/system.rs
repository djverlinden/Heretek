...
/// Voegt alle systeem-gerelateerde commando's toe
pub fn add_commands(commands: &mut ProxmoxCommands) {
    // Controleer of commando's al geladen zijn
    if commands.get_response(8, "pvecm status").is_some() {
        return;
    }

    // Laad alle versies uit het YAML bestand
    yaml::load_all_versions("system", commands);
}
...
mod tests {
    ...
    fn test_system_commands() {
        ...
        if !commands.version_commands.is_empty() {
            ...
            assert!(commands.get_response(version, "pvecm status").is_some());
        }
    }
}